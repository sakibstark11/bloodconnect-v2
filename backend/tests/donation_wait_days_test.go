...
func TestDonationWaitDays(t *testing.T) {
	...
	err = ts.reqService.RespondToRequest(ctx2, reqID2, domain.ActionStatusAccepted)
	if err == nil {
		t.Errorf("Expected User 2 to be REJECTED when accepting Request 2 within wait days, but it succeeded")
	}

	err = ts.reqService.RespondToRequest(ctx2, reqID2, domain.ActionStatusDeclined)
	if err != nil {
	...
	err = ts.reqService.RespondToRequest(ctx2, reqID1, domain.ActionStatusDeclined)
	if err != nil {
...
func TestUnauthorizedDonatedAction(t *testing.T) {
	...
	err := ts.reqService.RespondToRequest(ctx2, reqID1, domain.ActionStatusDonated)
	if err == nil {
